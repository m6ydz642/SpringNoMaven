...
<script type="text/javascript">
 ...
 function fn_contentDelete(board_id){
		...
		var url = "${pageContext.request.contextPath}/virutaloarddelete"; 
		var virutal_name = window.location.search;
		var split;
		split = virutal_name.split("&");
		
		// 가상 게시판 글삭제로 이동
		
	//	url = url + split[0] + "&board_id="+board_id; // 컨트롤러에 인자전달용
		url = url + split[0]; // 컨트롤러에 인자전달용
		alert (url);
